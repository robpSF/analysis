...
if uploaded_file:
    ...
    # Convert 'created' and 'updated' columns to datetime and strip the time component
    df['created'] = pd.to_datetime(df['created']).dt.date
    df['updated'] = pd.to_datetime(df['updated']).dt.date

    # Ensure there are valid dates before calculating the range
    if not df['created'].empty:
    ...
    if min_date and max_date:
        ...
        df['week_created'] = df['created'] - pd.to_timedelta(pd.to_datetime(df['created']).dt.weekday, unit='D')
        df['week_updated'] = df['updated'] - pd.to_timedelta(pd.to_datetime(df['updated']).dt.weekday, unit='D')
        ...
        st.write("No valid data available to set up a date range.")
